...
const VacinationRoutes = require("./Routes/Vacination");
...
app.use("/api/vacination", VacinationRoutes);
...
app.get("/searchDoc/:key", async (req, resp) => {
  let data = await Document.find({
    $or: [{ nom_document: { $regex: req.params.key } }],
  });
  resp.send(data);
});
app.get("/searchMedicament/:key", async (req, resp) => {
  let data = await Medicament.find({
    $or: [{ nom_medicament: { $regex: req.params.key } }],
  });
  resp.send(data);
});
