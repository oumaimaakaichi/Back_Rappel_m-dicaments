...
route.get("/medicaments/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Erreur lors de la recherche du médicament par ID" });
  }
});
route.put("/medicamentsUpdate/:id", async (req, res) => {
  try {
    ...
    medicament = await Medicament.findByIdAndUpdate(id, req.body, {
      new: true,
    });
    ...
    console.error(
      "Erreur lors de la mise à jour du médicament par ID :",
      error
    );
    res
      .status(500)
      .json({ error: "Erreur lors de la mise à jour du médicament par ID" });
  }
});
...
route.get("/medUtilisateur/:utilisateurId", async (req, res) => {
  try {
    const contacts = await Medicament.find({
